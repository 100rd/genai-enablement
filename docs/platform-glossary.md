...
| **PlatformPath** | A closed, versioned, executable A-to-Z contract that realizes or operates one PlatformProduct by composing EntityClasses, policies, delivery mechanisms, Conditions of Done, and compensation. | may prototype candidate paths | compiles a pinned approved path into a WorkOrder DAG | definitions originate in the owning platform repo; indexed by Omniscience |
...
| **PlatformPath lifecycle** | `draft -> experimental -> validated -> approved -> deprecated`. A WorkOrder pins one exact revision; a new version never changes active work or existing instances silently. |
