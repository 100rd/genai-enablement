...
**Status:** Adopted working definition · 2026-07-07
...
|---|---|---|---|---|
| ① | **Prevention** | security **verifier family** in the change-gate: `secret_in_change` (BLOCK), `iac_security_regression`, `image_cve_gate`, `privileged_workload` — catch an insecure change *before* it lands | no | sre-harness change-gate + gate CLI (ADR-0003 GATED / Stage 2) |
| ② | **Detection** | security **detector family** in Sentinel: IAM drift vs baseline, CVE-in-SBOM, exposed-surface-vs-policy, secret-in-state (defense-in-depth) — anomaly / new-class / risk over the running system | no | Sentinel (ADR-0001), `DEFAULT_SECURITY_DETECTORS` family |
...
- **Detection degrades OPEN.** A Sentinel security detector that loses the hub falls back to direct
  sources (AWS / registry / kubectl) — slower, still functional — **and detector liveness is
  monitored**, because a detector that suddenly reports nothing may have been *disabled by an
...
**Built (reused as-is):** the harness deterministic core + eval + gate CLI (main); the Sentinel engine,
detector contract, dedup/rank, and lead-time eval; the action-tier table; the skills registry
mechanics. None of this is security-specific yet, and none needs re-building.
